...
#to convert the coloumn to numerical datatype because some algorithm want data only fed numeric data
...
df=df.dropna(subset=['Maximum Temperature ','Minimum Temperature '])
...
feature_cols = ['Maximum Temperature ', 'Minimum Temperature ', 'Average Temperature','Rainfall']
...
#to add the predicted values to the original datafram where the dataframe have the nan values
...
df=df[['Year','Month','Day','Maximum Temperature ','Minimum Temperature ','Rainfall','Average Temperature','water Discharge']]
df.head()
...
for month in df['Month'].unique():
    ...
    plt.scatter(monthly_data['Month'], monthly_data['Minimum Temperature '], label=calendar.month_abbr[int(month)])
...
#no need of these columns
df=df.drop(['Year','Month','Day'],axis=1)
x=df[['Maximum Temperature ','Minimum Temperature ','Average Temperature','Rainfall','Month_sin','Month_cos','Day_sin','Day_cos']]
